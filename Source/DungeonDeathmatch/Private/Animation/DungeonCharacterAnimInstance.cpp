...
void UDungeonCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if (OwningCharacter)
	{
		UCharacterAnimationComponent* AnimationComponent = Cast<UCharacterAnimationComponent>(OwningCharacter->GetComponentByClass(UCharacterAnimationComponent::StaticClass()));
		if (AnimationComponent)
		{
			MovementSpeed = AnimationComponent->GetMovementVelocity().Size();
			MovementDirection = AnimationComponent->GetMovementDirection();
			...
			bIsReorientingBody = AnimationComponent->GetIsReorientingBody();
			bIsCrouched = OwningCharacter->bIsCrouched;
			bIsJumping = AnimationComponent->GetIsJumping();
			bIsMontagePlaying = IsAnyMontagePlaying();
			bIsFreeLooking = AnimationComponent->GetIsFreeLooking();
			...
			AimYawTarget = AnimationComponent->GetAimYaw();
			...
			AimPitch = AnimationComponent->GetAimPitch();
		}
	}
}
...
void UDungeonCharacterAnimInstance::UpdateAnimationResources()
{
	if (OwningCharacter)
	{
		...
		if (AnimationComponent)
		{
			...
			UBlendSpace* NewStandingMovementBlendSpace = AnimationComponent->GetCombatStandingMovementBlendSpace();
			...
			UBlendSpace* NewCrouchingMovementBlendSpace = AnimationComponent->GetCombatCrouchingMovementBlendSpace();
			...
			UAnimSequence* NewJumpingAnimation = AnimationComponent->GetCombatJumpingAnimation();
			...
			UBlendSpace1D* NewFallingBlendSpace = AnimationComponent->GetCombatFallingBlendSpace();
			...
			UBlendSpace* NewLandingBlendSpace = AnimationComponent->GetCombatLandingBlendSpace();
